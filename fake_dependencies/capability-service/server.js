...
const app = express();
const jwtUtilitiesClient = new jwtUtilities();
...
// Returns true if authorized, returns false if not.
var authHandler = (req, res) => {
    var payload = {
        authorized: false,
        data: {}
    }
    ...
    var user = jwtUtilitiesClient.getUserInformation(authHeaderParts[1]);
...
// Not exposed through Blaster, must call this fake dependency directly, should be '::1:50900'.
app.get("/api/DEV/toggle_auth_handler", (req, res) => {
    useAuthHandler = !useAuthHandler;
    console.log("useAuthHandler has been set to:", useAuthHandler);
    res.status(200).send();
});

app.get("/api/v1/capabilities", (req, res) => {
    if (authHandler(req, res).authorized) {
